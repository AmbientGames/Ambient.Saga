...
2. **Progress** — there is no separate progress store: `QuestProgressEvaluator` counts matching transactions, **scoped to the current acceptance**. The scope starts at the accepting dialogue *session* (fixed 2026-07-04), so tokens granted earlier in the conversation that accepts the quest count; events from a previous completed session, or from a prior accept/abandon cycle, never count.
