...
internal sealed class GetBattleStateHandler : IRequestHandler<GetBattleStateQuery, BattleStateResult>
{
    ...
    public async Task<BattleStateResult> Handle(GetBattleStateQuery query, CancellationToken ct)
    {
        ...
        try
        {
            ...
            if (battleHasEnded && battleEndedTx.Data.TryGetValue(TransactionDataKeys.PlayerVictory, out var victoryStr))
            {
            ...
            else
            {
                ...
                else
                {
                    ...
                    var wasPlayerTurn = bool.Parse(lastTurn.Data[TransactionDataKeys.IsPlayerTurn]);
