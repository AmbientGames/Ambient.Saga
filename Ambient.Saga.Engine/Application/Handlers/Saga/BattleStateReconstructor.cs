...
internal static class BattleStateReconstructor
{
    internal static (Combatant avatar, Combatant enemy, int randomSeed, List<string> avatarAffinityRefs, Guid enemyCharacterInstanceId, List<Combatant> companions)
        Reconstruct(SagaTransaction battleStartedTx, SagaInstance instance, IWorld world)
    {
    ...
    private static void ParseSlots(SagaTransaction battleStartedTx, string dataKey, Combatant combatant)
    {
    ...
    /// reconstructed here).
    /// </summary>
    private static Combatant? ResolveActor(SagaTransaction turnTx, bool isAvatarTurn, Combatant avatarCombatant, Combatant enemyCombatant)
    {
        if (isAvatarTurn)
    ...
    private static float ParseFloat(SagaTransaction tx, string dataKey)
    {
