...
public class HotbarService
{
    ...
    /// <summary>
    /// Assigns an item to a hotbar slot. The slot identity is the ref (any variation folded in).
    /// </summary>
    ...
    private async Task PerformSlotActionAsync(AvatarBase avatar, HotbarSlot slot)
    {
        ...
        switch (slot.ItemType)
        {
            ...
            case HotbarItemType.Block:
                ...
                var blockDef = world?.BlockProvider?.GetBlockByRefName(BlockRefVariation.BaseRef(slot.RefName));
