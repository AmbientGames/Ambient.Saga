...
public class VictoryLootStateMachineTests
{
    ...
    private static SagaStateMachine CreateStateMachine(World world)
        => new(world.SagaArcLookup["BanditArc"], world.SagaTriggersLookup["BanditArc"], world);

    private static SagaTransaction CreateTransaction(SagaTransactionType type, long seq, string avatarId, Dictionary<string, string> data)
        => new()
        {
    ...
    private static SagaTransaction Spawn(long seq, Guid instanceId, string avatarId = "system")
        => CreateTransaction(SagaTransactionType.CharacterSpawned, seq, avatarId, new Dictionary<string, string>
        {
            [TransactionDataKeys.CharacterRef] = "Bandit",
            [TransactionDataKeys.CharacterInstanceId] = instanceId.ToString()
        });

    private static SagaTransaction Defeat(long seq, Guid instanceId, string victorAvatarId)
        => CreateTransaction(SagaTransactionType.CharacterDefeated, seq, victorAvatarId, new Dictionary<string, string>
        {
            [TransactionDataKeys.CharacterInstanceId] = instanceId.ToString(),
            [TransactionDataKeys.CharacterRef] = "Bandit",
    ...
    private static SagaTransaction Take(long seq, Guid instanceId, string avatarId, int quantity = 1)
        => CreateTransaction(SagaTransactionType.ItemTraded, seq, avatarId, new Dictionary<string, string>
        {
            [TransactionDataKeys.CharacterInstanceId] = instanceId.ToString(),
    ...
    public void Spawn_SeedsCurrentInventoryFromInteractableLoot_NotCapabilities()
    {
        ...
        var state = CreateStateMachine(world).Replay(new List<SagaTransaction> { Spawn(1, instanceId) });
    ...
    public void CharacterDefeated_RecordsVictorOnSpawnInstance()
    {
        ...
        var state = CreateStateMachine(world).Replay(new List<SagaTransaction>
        {
            Spawn(1, instanceId),
