...
public class AvatarProgressRepository : IAvatarProgressRepository
{
    ...
    public void ProjectTransactions(Guid avatarId, string arcRef, IReadOnlyList<ArcTransaction> transactions)
    {
        foreach (var tx in transactions)
        {
            switch (tx.Type)
            {
                case ArcTransactionType.QuestTokenAwarded:
                    ProjectQuestToken(avatarId, arcRef, tx);
                    break;

                case ArcTransactionType.QuestAccepted:
                    ProjectQuestAccepted(avatarId, arcRef, tx);
                    break;
    ...
    /// </summary>
    public void ReverseTransactions(Guid avatarId, string arcRef, IReadOnlyList<ArcTransaction> reversedOriginals)
    {
    ...
    private void ProjectQuestToken(Guid avatarId, string arcRef, ArcTransaction tx)
    {
    ...
    private void ProjectQuestAccepted(Guid avatarId, string arcRef, ArcTransaction tx)
    {
    ...
    private void ProjectQuestCompleted(Guid avatarId, ArcTransaction tx)
    {
        var questRef = tx.Data.GetValueOrDefault(TransactionDataKeys.QuestRef);
        if (string.IsNullOrEmpty(questRef)) return;
    ...
    private void ProjectQuestAbandoned(Guid avatarId, ArcTransaction tx)
    {
        var questRef = tx.Data.GetValueOrDefault(TransactionDataKeys.QuestRef);
        if (string.IsNullOrEmpty(questRef)) return;
    ...
    private void ProjectQuestStageAdvanced(Guid avatarId, ArcTransaction tx)
    {
        var questRef = tx.Data.GetValueOrDefault(TransactionDataKeys.QuestRef);
        if (string.IsNullOrEmpty(questRef)) return;
    ...
    private void ProjectCharacterDefeated(Guid avatarId, ArcTransaction tx)
    {
    ...
    private void ProjectReputationChanged(Guid avatarId, ArcTransaction tx)
    {
    ...
    private void ProjectTraitAssigned(Guid avatarId, ArcTransaction tx)
    {
        var characterRef = tx.Data.GetValueOrDefault(TransactionDataKeys.CharacterRef);
    ...
    private void ReverseQuestToken(Guid avatarId, ArcTransaction tx)
    {
        var tokenRef = tx.Data.GetValueOrDefault(TransactionDataKeys.QuestTokenRef);
        if (string.IsNullOrEmpty(tokenRef)) return;
    ...
    private void ReverseQuestAccepted(Guid avatarId, ArcTransaction tx)
    {
        var questRef = tx.Data.GetValueOrDefault(TransactionDataKeys.QuestRef);
        if (string.IsNullOrEmpty(questRef)) return;
    ...
    private void ReverseQuestTerminalStatus(Guid avatarId, ArcTransaction tx)
    {
        var questRef = tx.Data.GetValueOrDefault(TransactionDataKeys.QuestRef);
        if (string.IsNullOrEmpty(questRef)) return;
    ...
    private void ReverseCharacterDefeated(Guid avatarId, ArcTransaction tx)
    {
        var characterRef = tx.Data.GetValueOrDefault(TransactionDataKeys.CharacterRef);
        if (string.IsNullOrEmpty(characterRef)) return;
    ...
    private void ReverseReputationChanged(Guid avatarId, ArcTransaction tx)
    {
        var factionRef = tx.Data.GetValueOrDefault(TransactionDataKeys.FactionRef);
        if (string.IsNullOrEmpty(factionRef)) return;
        if (!int.TryParse(tx.Data.GetValueOrDefault(TransactionDataKeys.Amount), out var amount)) return;
    ...
    private void ReverseTraitAssigned(Guid avatarId, ArcTransaction tx)
    {
    ...
    private void ProjectTraitRemoved(Guid avatarId, ArcTransaction tx)
    {
        var characterRef = tx.Data.GetValueOrDefault(TransactionDataKeys.CharacterRef);
        var traitType = tx.Data.GetValueOrDefault(TransactionDataKeys.TraitType);
        if (string.IsNullOrEmpty(characterRef) || string.IsNullOrEmpty(traitType)) return;
