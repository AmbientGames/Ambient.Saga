    ...
    where TRequest : notnull
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!isValid)
        {
            ...
            // If response is ArcCommandResult, return failure result
            if (typeof(TResponse) == typeof(ArcCommandResult))
            {
                var failureResult = ArcCommandResult.Failure(Guid.Empty, $"Validation failed: {errors}");
