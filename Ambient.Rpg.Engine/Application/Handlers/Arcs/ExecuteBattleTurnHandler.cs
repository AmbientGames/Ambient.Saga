...
/// </summary>
internal sealed class ExecuteBattleTurnHandler : IRequestHandler<ExecuteBattleTurnCommand, ArcCommandResult>
{
    ...
    public async Task<ArcCommandResult> Handle(ExecuteBattleTurnCommand command, CancellationToken ct)
    {
        ...
        try
        {
            ...
            if (command.Avatar?.Capabilities != null)
            {
                ...
                // Mid-battle consumable use folds the same way (absolute post-use
                // quantities) — used potions used to resurrect on reload
                BattleStateReconstructor.ApplyRecordedConsumableQuantities(instance, command.BattleInstanceId, avatarCombatant);
            }
            ...
            var newTransactions = new List<ArcTransaction>();
            ...
            if (pendingTellTx != null)
            {
                ...
                var tellTarget = BattleTransactionHelper.ResolveTellTarget(
                    pendingTellTx, battleEngine.GetAvatar(), battleEngine.GetCompanions());
                if (battleEngine.BeginAttackWithTell(battleEngine.GetEnemy(), tellTarget, tellRefName, tellBaseDamage))
                {
            ...
            else
            {
                ...
                if (battleEngine.State == BattleState.EnemyTurn)
                {
                    ...
                    if (battleEngine.State == BattleState.AwaitingReaction && battleEngine.PendingAttack != null)
                    {
                        ...
                        // Persist WHO the tell is aimed at (H6). SelectEnemyTarget can pick a
                        // companion and the base damage is computed against that target's Defense;
                        // rebuilding with the avatar hardcoded used to land the hit on the avatar.
                        // Avatar-targeted tells carry no key — ResolveTellTarget defaults to avatar.
                        if (!ReferenceEquals(pending.Target, battleEngine.GetAvatar()))
                            tellTx.Data[BattleTransactionHelper.TellTargetRefKey] = pending.Target.RefName;
        ...
        catch (Exception ex)
        {
            ...
            return ArcCommandResult.Failure(Guid.Empty, $"Error executing battle turn: {ex.Message}");
        }
    }
    ...
    /// </summary>
    private async Task<ArcCommandResult> CommitAndFinish(
    ...
    /// </summary>
    private static void ApplyRecordedEquipmentConditions(ArcInstance instance, Guid battleInstanceId, Combatant avatarCombatant)
    {
