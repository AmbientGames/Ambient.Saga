...
/// satisfying <see cref="ArcTransaction"/> therefore lives in a DIFFERENT
/// <see cref="ArcInstance"/> than the one that accepted the quest. Evaluating an
...
{
    /// <summary>
    /// Loads every Arc instance for the avatar and merges their committed
    /// transactions into one chronological, coherently-sequenced log.
    /// </summary>
    public static async Task<List<ArcTransaction>> BuildAsync(
        Guid avatarId,
    ...
    ///
    /// 2. <b>Re-sequencing.</b> <see cref="ArcTransaction.SequenceNumber"/> is a
    ...
    /// </remarks>
    public static List<ArcTransaction> Build(Guid avatarId, IEnumerable<ArcInstance> instances)
    {
        ...
        var merged = new List<ArcTransaction>(ordered.Count);
        long sequence = 1;
    ...
    /// <summary>
    /// Shallow copy that only overrides <see cref="ArcTransaction.SequenceNumber"/>.
    ...
    /// <see cref="ArcTransaction.Data"/>, so sharing that dictionary reference is safe.
    /// </summary>
    private static ArcTransaction CopyWithSequence(ArcTransaction source, long sequenceNumber) => new()
    {
