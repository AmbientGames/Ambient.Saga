...
/// </summary>
internal sealed class ApplyBattleDialogueEffectsHandler : IRequestHandler<ApplyBattleDialogueEffectsCommand, ArcCommandResult>
{
    ...
    public async Task<ArcCommandResult> Handle(ApplyBattleDialogueEffectsCommand command, CancellationToken ct)
    {
        try
        {
            ...
            if (!string.IsNullOrEmpty(command.EndBattleResult))
            {
                    ...
                    _world.CharactersLookup.TryGetValue(enemyCombatant.RefName, out var enemyCharacter))
                {
                    ...
                    // Award the template's GivesQuestTokenOnDefeat tokens — a scripted
                    // victory defeats the enemy for quest purposes, so it must also
                    // grant defeat tokens (same guard semantics as DefeatCharacterHandler).
                    BattleEndTransactionFactory.AppendDefeatTokenGrants(
                        command.AvatarId, enemyCharacterInstanceId, enemyCharacter, instance, newTransactions);
                }
            }
        ...
        catch (Exception ex)
        {
            ...
            return ArcCommandResult.Failure(Guid.Empty, $"Error applying battle dialogue effects: {ex.Message}");
        }
    }
}
