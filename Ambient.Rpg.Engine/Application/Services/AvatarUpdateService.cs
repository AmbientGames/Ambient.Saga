...
namespace Ambient.Rpg.Engine.Application.Services;
...
public class AvatarUpdateService : IAvatarUpdateService
{
        ...
        CancellationToken ct = default)
    {
        ...
        var transaction = arcInstance.GetCommittedTransactions()
            .FirstOrDefault(t => t.TransactionId == tradeTransactionId && t.Type == ArcTransactionType.ItemTraded);
        ...
        // Degradables keep one inventory entry per ITEM (crafting produces honest duplicates
        // of a ref): buying appends entries, selling removes exactly the traded count — a
        // whole-ref RemoveAll used to vanish every duplicate when one was sold.
        else if (_world.EquipmentLookup.ContainsKey(itemRef))
        {
            ...
            {
                RemoveEntries(equipment, e => e.EquipmentRef == itemRef, quantity);
            }
        ...
        else if (_world.ToolsLookup.ContainsKey(itemRef))
        {
            ...
            {
                RemoveEntries(tools, t => t.ToolRef == itemRef, quantity);
            }
        ...
        else if (_world.SpellsLookup.ContainsKey(itemRef))
        {
            ...
            {
                RemoveEntries(spells, s => s.SpellRef == itemRef, quantity);
            }
        ...
        CancellationToken ct = default)
    {
        ...
        var battleStartedTx = arcInstance.GetCommittedTransactions()
            .FirstOrDefault(t => t.TransactionId == battleStartedTransactionId && t.Type == ArcTransactionType.BattleStarted);
        ...
        var allBattleTurns = arcInstance.GetCommittedTransactions()
        ...
        CancellationToken ct = default)
    {
        ...
        var transaction = arcInstance.GetCommittedTransactions()
            .FirstOrDefault(t => t.TransactionId == effectTransactionId && t.Type == ArcTransactionType.EffectApplied);
        ...
        if (transaction.Data.TryGetValue(TransactionDataKeys.Experience, out var experienceStr) && float.TryParse(experienceStr, out var experience) && experience != 0.0f)
        {
            ...
            avatar.Stats.Level = Math.Max(avatar.Stats.Level,
                Domain.Progression.LevelCurve.GetLevelForExperience(avatar.Stats.Experience));
        }
    ...
    /// <summary>
    /// Removes up to <paramref name="count"/> entries matching the predicate — a sale of N
    /// duplicates removes exactly N, never the whole ref.
    /// </summary>
    private static void RemoveEntries<T>(List<T> entries, Predicate<T> match, int count)
    {
        for (var removed = 0; removed < count; removed++)
        {
            var index = entries.FindIndex(match);
            if (index < 0)
                return;
