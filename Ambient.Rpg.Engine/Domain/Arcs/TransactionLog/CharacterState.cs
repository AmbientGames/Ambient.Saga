...
/// <summary>
/// Runtime state of a character spawned in an arc instance.
...
public class CharacterState
{
    ...
    /// <summary>
    /// True when this character-state carries the named trait with a non-zero
    /// value. Boolean flags stored as null count as carried (present == true);
    /// an explicit Value="0" (e.g. Hostile="0" on a deliberately non-hostile
    /// authored NPC) counts as NOT carried. Mirrors the value-aware semantics of
    /// Character.CarriesTrait so a key-only presence check can never treat an
    /// explicitly-off trait as on.
    /// </summary>
    public bool CarriesTrait(string traitName)
        => Traits.TryGetValue(traitName, out var value) && value != 0;
}
