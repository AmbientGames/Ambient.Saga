...
/// Ise world: real Dialogue.xml / Quests.xml / Characters.xml / Saga.xml loaded through
...
/// quest log through RpgMainViewModel.QuestLog and the quest detail modal's
...
public class RealContentDialogueQuestE2ETests : IAsyncLifetime, IDisposable
{
    ...
    private const string ArcRef = "Kazahinominomiya";           // real Ise arc (first shrine)
    ...
    public async Task FullConversation_QuestOfferWalk_AcceptQuest_QuestActive_TokenScoped_SessionSealed()
    {
        ...
        var instance = await _host.Repositories.ArcRepository.GetOrCreateInstanceAsync(avatar.AvatarId, ArcRef);
        ...
        await _host.Repositories.ArcRepository.AddAndCommitTransactionsAsync(
            instance.InstanceId, new List<ArcTransaction> { spawnTx });
        ...
        var lastStarted = committed.Where(t => t.Type == ArcTransactionType.DialogueStarted)
            .OrderBy(t => t.SequenceNumber).Last();
        var lastCompleted = committed.Where(t => t.Type == ArcTransactionType.DialogueCompleted)
        ...
        var acceptedTx = Assert.Single(committed, t => t.Type == ArcTransactionType.QuestAccepted);
        ...
        Assert.True(arcState!.ActiveQuests.ContainsKey(QuestRef), "Quest must be active in the arc that accepted it");
        ...
        var tokenTx = Assert.Single(committed, t =>
            t.Type == ArcTransactionType.QuestTokenAwarded &&
