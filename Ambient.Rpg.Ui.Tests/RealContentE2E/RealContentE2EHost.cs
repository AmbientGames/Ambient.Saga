...
///
/// - REAL world content: the vendored Arc-flavor worlds (Ise/Kagoshima) copied to
...
///   QuestStageProgressionBehavior (mirrors Ambient.Archimedea Program.cs).
...
///   arc repository — the cross-arc token seam).
/// - REAL RpgMainViewModel constructed exactly like the game constructs it, with the
///   providers initialized the way RpgMainViewModel.InitializeWorldDatabase does on
...
internal sealed class RealContentE2EHost : IDisposable
{
    ...
    public RpgMainViewModel ViewModel { get; private set; } = null!;
    ...
    public static async Task<RealContentE2EHost> CreateAsync(string worldRefName, string? existingTempDirectory = null)
    {
        ...
        host.TempDirectory = existingTempDirectory
            ?? Path.Combine(Path.GetTempPath(), $"AmbientArcUiE2E_{Guid.NewGuid():N}");
        ...
        // vendored Arc-flavor worlds.
        var gameSettings = new GameSettings(PublisherDefaults.PublisherFolder, "Arc");
        var contentPathResolver = new ContentPathResolver(gameSettings);

        var readModelRepository = new InMemoryArcReadModelRepository();

        // Providers are the same mutable singletons the game registers at startup and
        // initializes on world load (see Ambient.Archimedea Program.cs).
        var worldProvider = new WorldProvider();
        var arcRepositoryProvider = new ArcInstanceRepositoryProvider();
        ...
        // handlers resolve these — the same query path RpgMainViewModel drives).
        ...
        services.AddSingleton(arcRepositoryProvider);
        ...
        services.AddTransient(_ => arcRepositoryProvider.Repository);
        ...
        services.AddSingleton<IArcReadModelRepository>(readModelRepository);
        ...
        // What RpgMainViewModel.InitializeWorldDatabase does on world load: the REAL
        // repository factory over the (temp-file) LiteDB, which also wires the
        // avatar-progress projection into the arc repository.
        ...
        arcRepositoryProvider.SetRepository(host.Repositories.ArcRepository);
        ...
        host.ViewModel = new RpgMainViewModel(
            worldProvider,
            arcRepositoryProvider,
