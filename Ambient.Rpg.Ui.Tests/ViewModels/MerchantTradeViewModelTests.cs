    ...
    public Task<TResponse> Send<TResponse>(IRequest<TResponse> request, CancellationToken cancellationToken = default)
    {
        ...
        if (request is TradeItemCommand tradeCmd)
        {
            ...
            if (tradeCmd.IsBuying)
            {
                ...
                {
                    var failureResult = ArcCommandResult.Failure(
                        Guid.Empty,
                        "Not enough money"
                    );
        ...
        // Handle GetArcStateQuery
        if (request is GetArcStateQuery)
        {
            var arcState = new ArcState
            {
                ArcRef = "TestArc",
                Status = ArcStatus.Active
            };
            return Task.FromResult((TResponse)(object)arcState);
        }
...
public class MerchantTradeViewModelTests
{
    ...
    private ArcInteractionContext CreateTestContext()
    {
        return new ArcInteractionContext
        {
            World = _world,
    ...
    {
        var contextWithNullWorld = new ArcInteractionContext
        {
            World = null,
            AvatarEntity = _avatar,
            ActiveCharacter = _merchant
    ...
    {
        var contextWithNullAvatar = new ArcInteractionContext
        {
            World = _world,
            AvatarEntity = null,
            ActiveCharacter = _merchant
    ...
    [Fact]
    public async Task BuyItemCommand_WithNullArcRef_FiresStatusMessage()
    {
        var contextWithNullArc = new ArcInteractionContext
        {
            World = _world,
            AvatarEntity = _avatar,
            ActiveCharacter = _merchant,
        ...
        var viewModel = new MerchantTradeViewModel(contextWithNullArc, _mediator);
