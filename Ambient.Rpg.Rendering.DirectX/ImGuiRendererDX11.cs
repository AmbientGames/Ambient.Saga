...
/// Includes keyboard navigation, theming, DPI-aware fonts, and native Windows clipboard support.
/// </summary>
public class ImGuiRendererDX11 : IDisposable
{
    ...
    private readonly List<ShaderResourceView> _userTextures = new();
    ...
    /// <summary>
    /// Loads a PNG file as a DX11 texture and returns an IntPtr usable with ImGui.Image/ImageButton.
    /// </summary>
    public IntPtr LoadTextureFromFile(string filePath)
    {
        ...
        unsafe
        {
            ...
            for (int i = 0; i < w * h; i++)
            {
                pixels[i * 4 + 0] = src[i * 4 + 2]; // R (from BGRA)
        ...
        try
        {
            ...
            using var texture = new Texture2D(_device, desc, new DataRectangle(pinned.AddrOfPinnedObject(), w * 4));
            var srv = new ShaderResourceView(_device, texture);
            _userTextures.Add(srv);
    ...
    public void Dispose()
    {
        ...
        foreach (var srv in _userTextures) srv.Dispose();
        _userTextures.Clear();
