...
/// </summary>
public class ArcSyncReconciliationTests : IDisposable
{
    private readonly MemoryStream _stream = new();
    private readonly LiteDatabase _db;
    private readonly ArcInstanceRepository _repo;
    ...
    public ArcSyncReconciliationTests()
    {
        _db = new LiteDatabase(_stream);
        _repo = new ArcInstanceRepository(_db);
    ...
    private static ArcTransaction CreateLocal(ArcTransactionType type, Guid avatarId, Dictionary<string, string>? data = null)
        => new()
        {
    ...
    private static ArcTransaction CreateImported(ArcTransactionType type, Guid avatarId, long sequenceNumber, Dictionary<string, string>? data = null)
        => new()
        {
            TransactionId = Guid.NewGuid(),
            Type = type,
    ...
    /// <summary>The push predicate ArcSyncService uses (mirrored here — Engine.Core is not referenced).</summary>
    private static List<ArcTransaction> SelectForPush(IEnumerable<ArcTransaction> transactions)
        => transactions
    ...
    public async Task LocalCommit_SurvivesImportOfHigherForeignSequences_AndStillPushes()
    {
        ...
        var local = CreateLocal(ArcTransactionType.EntityInteracted, _avatarA,
        ...
        var foreign1 = CreateImported(ArcTransactionType.EntityInteracted, _avatarB, 5,
            new Dictionary<string, string> { [TransactionDataKeys.FeatureRef] = "well" });
        var foreign2 = CreateImported(ArcTransactionType.EntityInteracted, _avatarB, 6,
            new Dictionary<string, string> { [TransactionDataKeys.FeatureRef] = "well" });
    ...
    public async Task ImportedTransactions_ArePersistedAsImported_AndNeverSelectedForPush()
    {
        ...
        var foreign = CreateImported(ArcTransactionType.EntityInteracted, _avatarB, 3);
    ...
    public async Task RejectAndCompensate_ImportedTransaction_IsUntouched()
    {
        ...
        var foreign = CreateImported(ArcTransactionType.EntityInteracted, _avatarB, 4);
        await _repo.ImportTransactionsAsync(_avatarA, instance.InstanceId, [foreign]);
    ...
    public async Task RejectAndCompensate_LocalTransactions_MarksRejected_WritesReversals_AndUnprojects()
    {
        ...
        var token = CreateLocal(ArcTransactionType.QuestTokenAwarded, _avatarA,
            new Dictionary<string, string> { [TransactionDataKeys.QuestTokenRef] = "token-f2" });
        var rep = CreateLocal(ArcTransactionType.ReputationChanged, _avatarA,
        ...
        {
            Assert.Equal(ArcTransactionType.TransactionReversed, r.Type);
    ...
    public async Task RejectAndCompensate_IsIdempotent()
    {
        ...
        var all = await _repo.GetTransactionsAsync(instance.InstanceId);
        Assert.Single(all, t => t.Type == ArcTransactionType.TransactionReversed);
    }
    ...
    public async Task Reimport_SamePullPayload_IsIdempotent()
    {
        ...
        var defeat = CreateImported(ArcTransactionType.CharacterDefeated, _avatarA, 1,
    ...
    public async Task MarkTransactionsSynced_OnlyMovesLocalUnsynced_AndStampsServerTimestamp()
    {
        ...
        var foreign = CreateImported(ArcTransactionType.EntityInteracted, _avatarB, 9);
