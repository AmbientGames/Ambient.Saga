...
namespace Ambient.Rpg.Engine.Tests.Helpers;
...
{
    private readonly Dictionary<string, ArcInstance> _arcInstances = new();
    private readonly Dictionary<string, AvatarEntity> _avatars = new();
    private readonly List<AvatarDiscovery> _discoveries = new();

    public void AddArcInstance(string avatarId, string templateRef, ArcInstance instance)
    {
        var key = $"{avatarId}:{templateRef}";
        _arcInstances[key] = instance;
    }

    public Task<ArcInstance?> GetArcInstanceAsync(string avatarId, string templateRef)
    {
        var key = $"{avatarId}:{templateRef}";
        _arcInstances.TryGetValue(key, out var instance);
