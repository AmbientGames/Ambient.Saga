...
/// TradeItemHandler emits these when avatar persistence fails after the arc-side
...
public class TransactionReversalTests
{
    ...
    private static ArcStateMachine CreateStateMachine(World world, IArcMetrics? metrics = null)
        => new(world.ArcLookup["VillageMerchant"], world.ArcTriggersLookup["VillageMerchant"], world, metrics: metrics);

    private static ArcTransaction CreateTransaction(ArcTransactionType type, long sequenceNumber, Dictionary<string, string> data)
        => new()
        {
    ...
    private static ArcTransaction CreateSpawnTransaction(long sequenceNumber)
        => CreateTransaction(ArcTransactionType.CharacterSpawned, sequenceNumber, new Dictionary<string, string>
        {
            [TransactionDataKeys.CharacterRef] = "Merchant",
            [TransactionDataKeys.CharacterInstanceId] = MerchantInstanceId.ToString()
        });

    private static ArcTransaction CreateReversalTransaction(long sequenceNumber, ArcTransaction original)
        => CreateTransaction(ArcTransactionType.TransactionReversed, sequenceNumber, new Dictionary<string, string>
        {
    ...
    public void Replay_ReversedItemTraded_RestoresMerchantStock()
    {
        ...
        var trade = CreateTransaction(ArcTransactionType.ItemTraded, 2, new Dictionary<string, string>
        {
            [TransactionDataKeys.CharacterInstanceId] = MerchantInstanceId.ToString(),
        ...
        var stateWithoutReversal = stateMachine.Replay(new List<ArcTransaction>
        {
        ...
        var state = stateMachine.Replay(new List<ArcTransaction>
        {
    ...
    public void Replay_ReversedReputationChanged_RestoresReputation()
    {
        ...
        var repChange = CreateTransaction(ArcTransactionType.ReputationChanged, 1, new Dictionary<string, string>
        {
    ...
    public void Replay_ReversedQuestTokenAwarded_RemovesToken()
    {
        ...
        var award = CreateTransaction(ArcTransactionType.QuestTokenAwarded, 1, new Dictionary<string, string>
        {
            [TransactionDataKeys.QuestTokenRef] = "token-reversal-test"
        });

        // Sanity: without the reversal the token is present
        var stateWithoutReversal = stateMachine.Replay(new List<ArcTransaction> { award });
    ...
    public void Replay_ReversedQuestAccepted_RemovesFromActiveQuests()
    {
        ...
        var accept = CreateTransaction(ArcTransactionType.QuestAccepted, 1, new Dictionary<string, string>
        {
            [TransactionDataKeys.QuestRef] = "REVERSAL_QUEST"
        });

        // Sanity: without the reversal the quest is active
        var stateWithoutReversal = stateMachine.Replay(new List<ArcTransaction> { accept });
    ...
    public void Replay_ReversedQuestCompleted_RemovesFromCompleted_WithoutReactivating()
    {
        ...
        var accept = CreateTransaction(ArcTransactionType.QuestAccepted, 1, new(questData));
        var complete = CreateTransaction(ArcTransactionType.QuestCompleted, 2, new(questData));
        ...
        var reaccepted = stateMachine.Replay(new List<ArcTransaction>
        {
            accept, complete, CreateReversalTransaction(3, complete),
            CreateTransaction(ArcTransactionType.QuestAccepted, 4, new(questData))
        });
    ...
    public void Replay_ReversedTraitAssigned_RemovesTraitFromTemplateAndLiveInstance()
    {
        ...
        var assign = CreateTransaction(ArcTransactionType.TraitAssigned, 2, new Dictionary<string, string>
        {
            [TransactionDataKeys.CharacterRef] = "Merchant",
    ...
    public void Replay_ReversedQuestStageAdvanced_HasNoInverseFold_AndSkipsQuietly()
    {
        ...
        var questData = new Dictionary<string, string> { [TransactionDataKeys.QuestRef] = "REVERSAL_QUEST" };
        var accept = CreateTransaction(ArcTransactionType.QuestAccepted, 1, new(questData));
        var advance = CreateTransaction(ArcTransactionType.QuestStageAdvanced, 2, new Dictionary<string, string>
        {
            [TransactionDataKeys.QuestRef] = "REVERSAL_QUEST",
    ...
    public void Replay_TransactionReversed_NeverHitsUnknownTypeMetric()
    {
        ...
        var danglingReversal = CreateTransaction(ArcTransactionType.TransactionReversed, 4, new Dictionary<string, string>
        {
            [TransactionDataKeys.ReversedTransactionId] = Guid.NewGuid().ToString(),
            [TransactionDataKeys.Reason] = "Avatar persistence failed: test",
            [TransactionDataKeys.OriginalType] = ArcTransactionType.ItemTraded.ToString()
        });

        stateMachine.Replay(new List<ArcTransaction>
        {
