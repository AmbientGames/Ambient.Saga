...
namespace Ambient.Rpg.Engine.Tests.Rpg.Trade;
...
public class TradeEngineTests
{
    ...
    [Fact]
    public void CalculateSellPrice_ReturnsBaseValue()
    {
    ...
    /// <summary>
    /// A block-style item whose varieties carry authored ValueMultipliers (RareIngots:
    /// silver 1.5x, gold 3x the base metal price) — what each variety is worth.
    /// </summary>
    private sealed class VariantValuedItem : ITradeable
    {
        public string RefName => "RareIngots";
        public string DisplayName => "Ingot";
        public int BaseValue => 40;
        public float MerchantMarkupMultiplier => 2f;
        public int GetBaseValue(int variant) => variant switch
        {
            0 => 60,  // silver: 40 * 1.5
            1 => 120, // gold: 40 * 3
            _ => BaseValue
    ...
    [Fact]
    public void CalculatePrices_VarietyWithValueMultiplier_ScalesBothSidesOfTheCounter()
    {
        var ingots = new VariantValuedItem();

        // ValueMultiplier is worth: it raises the trade-in price AND the buy price alike,
        // so the shop's 2x selling multiplier applies identically to every variety.
        Assert.Equal(60, _engine.CalculateSellPrice(ingots, variant: 0));
        Assert.Equal(120, _engine.CalculateBuyPrice(ingots, isMerchant: true, variant: 0));
        Assert.Equal(120, _engine.CalculateSellPrice(ingots, variant: 1));
        Assert.Equal(240, _engine.CalculateBuyPrice(ingots, isMerchant: true, variant: 1));
    }

    [Fact]
    public void CalculatePrices_ItemWithoutValueMultipliers_IgnoresVariant()
    {
        var equipment = _world.Gameplay.Equipment[0]; // Iron Sword: 100 * 1.5

        Assert.Equal(_engine.CalculateBuyPrice(equipment, isMerchant: true),
                     _engine.CalculateBuyPrice(equipment, isMerchant: true, variant: 3));
        Assert.Equal(_engine.CalculateSellPrice(equipment),
                     _engine.CalculateSellPrice(equipment, variant: 3));
    }

    [Fact]
    public void GetAvailableItems_Tools_GroupsIdenticalDuplicates()
    {
        // Crafted extras (same ref AND condition) collapse into one row with a quantity;
        // a worn copy keeps its own row.
        var inventory = new ItemCollection
        {
            Tools = new[]
            {
                ...
                new ToolEntry { ToolRef = "pickaxe", Condition = 1f },
                new ToolEntry { ToolRef = "pickaxe", Condition = 0.5f }
            }
        };

        var items = _engine.GetAvailableItems(inventory, "Tools", isBuying: false);

        Assert.Equal(2, items.Count);
        Assert.Equal(2, items.Single(i => i.Condition == 1f).Quantity);
        Assert.Null(items.Single(i => i.Condition == 0.5f).Quantity);
    }

    [Fact]
    public void SellItem_DuplicateToolStack_MovesExactlyTheStack()
    {
        ...
        var buyer = new ItemCollection();
        var row = new TradeItemInfo(_world.Gameplay.Tools[0], price: 75, quantity: 2, condition: 1f);

        var result = _engine.SellItem(avatar, buyer, row);

        Assert.True(result.Success);
        var remaining = Assert.Single(avatar.Capabilities.Tools); // the worn one stays
        Assert.Equal(0.5f, remaining.Condition);
        Assert.Equal(2, buyer.Tools.Length);
    }
    ...
    public void GetAvailableItems_Equipment_ReturnsSellPrices()
    {
        ...
        Assert.Equal(2, items.Count);
        Assert.Equal("iron_sword", items[0].Item.RefName);
        Assert.Equal(100, items[0].Price); // BaseValue
    }
    ...
    public void CompleteTradeScenario_BuyAndSell()
    {
        ...
        // Avatar sells the sword back for 100 (BaseValue)
