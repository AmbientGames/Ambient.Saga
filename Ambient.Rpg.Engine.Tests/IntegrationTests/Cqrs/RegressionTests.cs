...
public class RegressionTests : IDisposable
{
    ...
    public async Task Regression_CharacterDuplication_MultiplePositionUpdates_OnlySpawnOnce()
    {
        ...
        // FIX: Added check for ArcTriggerStatus.Active (not just Completed)
        ...
        var firstSpawnCount = instance.GetCommittedTransactions()
            .Count(t => t.Type == ArcTransactionType.CharacterSpawned);
        ...
        instance = await _repository.GetOrCreateInstanceAsync(avatarId, arcRef);
    ...
    public async Task Regression_TransactionsListReload_ConcurrentAccess_NoException()
    {
        ...
        var reloadTasks = Enumerable.Range(0, 20).Select(_ =>
            Task.Run(async () =>
            {
                var reloaded = await repository.GetOrCreateInstanceAsync(avatarId, arcRef);
    ...
    public async Task Regression_TriggerActivation_IdempotentOnReentry()
    {
        ...
        var initialTriggerCount = instance.GetCommittedTransactions()
            .Count(t => t.Type == ArcTransactionType.TriggerActivated);
    ...
    public async Task Regression_SequenceNumbers_StrictlyIncreasing()
    {
        ...
        var results = new List<ArcCommandResult>();
    ...
    private World CreateRegressionTestWorld()
    {
        ...
        var duplicationArc = new Arc
        {
            RefName = "DuplicationTestArc",
            DisplayName = "Duplication Test Arc",
            Latitude = 35.0,
            Longitude = 139.0
        };
        ...
        var world = new World
        {
            ...
            WorldTemplate = new WorldTemplate
            {
                ...
                {
                    Saga = new[] { duplicationArc, raceArc },
        ...
        // Populate lookups
        world.ArcLookup[duplicationArc.RefName] = duplicationArc;
        world.ArcLookup[raceArc.RefName] = raceArc;
        world.ArcTriggersLookup[duplicationArc.RefName] = new List<ArcTrigger> { trigger };
        world.ArcTriggersLookup[raceArc.RefName] = new List<ArcTrigger>();
