...
public class InventoryCommandTests : IDisposable
{
    ...
    #region SharpenTool Tests

    private AvatarEntity CreateAvatarWithDullTool(Guid avatarId)
    {
        var avatar = CreateTestAvatar(avatarId);
        avatar.Stats!.Credits = 100;
        avatar.Capabilities!.Tools = new[]
        {
            new ToolEntry { ToolRef = "IronPick", Condition = 0.4f }
        };
        return avatar;
    }

    [Fact]
    public async Task SharpenTool_NegativeCost_IsRejected_AndMintsNothing()
    {
        // The cost is server-authoritative (ToolSharpening.CostCredits). A crafted
        // command with a negative cost must not pass the affordability check and
        // mint credits via Credits -= Cost.
        var avatarId = Guid.NewGuid();
        var avatar = CreateAvatarWithDullTool(avatarId);

        var result = await _mediator.Send(new SharpenToolCommand
        {
            AvatarId = avatarId,
            Avatar = avatar,
            ArcRef = "AvatarLife",
        ...
        Assert.Equal(0.4f, avatar.Capabilities!.Tools![0].Condition);
    }

    [Fact]
    public async Task SharpenTool_UnderpricedCost_IsRejected()
    {
        // A cheaper-than-canonical cost (including 0) is a forged discount
        var avatarId = Guid.NewGuid();
        var avatar = CreateAvatarWithDullTool(avatarId);
    ...
    [Fact]
    public async Task SharpenTool_CanonicalCost_RestoresConditionAndCharges()
    {
        var avatarId = Guid.NewGuid();
        var avatar = CreateAvatarWithDullTool(avatarId);
        ...
        var result = await _mediator.Send(new SharpenToolCommand
        {
            ...
            ArcRef = "AvatarLife",
            ToolRef = "IronPick",
            Cost = ToolSharpening.CostCredits
        });

        Assert.True(result.Successful, $"Sharpen failed: {result.ErrorMessage}");
        Assert.Equal(100 - ToolSharpening.CostCredits, avatar.Stats!.Credits);
        Assert.Equal(1.0f, avatar.Capabilities!.Tools![0].Condition);
    }
    ...
    public async Task EquipItem_ValidEquipment_CreatesEquipmentChangedTransaction()
    {
        ...
        var equipTx = instance.GetCommittedTransactions()
            .FirstOrDefault(t => t.Type == ArcTransactionType.EquipmentChanged);
    ...
    public async Task UseConsumable_ValidConsumable_CreatesConsumableUsedTransaction()
    {
        ...
        var useTx = instance.GetCommittedTransactions()
            .FirstOrDefault(t => t.Type == ArcTransactionType.ConsumableUsed);
    ...
    public async Task UseConsumable_MultipleUses_AllTracked()
    {
        ...
        var useTransactions = instance.GetCommittedTransactions()
            .Where(t => t.Type == ArcTransactionType.ConsumableUsed)
            .ToList();
