...
public class QuestCommandsIntegrationTests : IDisposable
{
    ...
    public async Task AcceptQuest_Success_CreatesQuestAcceptedTransaction()
    {
        ...
        var instance = await _repository.GetOrCreateInstanceAsync(_testAvatarId, "TEST_ARC");
        var transactions = instance.GetCommittedTransactions();
        var questAccepted = transactions.FirstOrDefault(t => t.Type == ArcTransactionType.QuestAccepted);
    ...
    public async Task ProgressQuestObjective_ThresholdMet_CreatesObjectiveCompletedTransaction()
    {
        ...
        instance = await _repository.GetOrCreateInstanceAsync(_testAvatarId, "TEST_ARC");
        var transactions = instance.GetCommittedTransactions();
        var objectiveCompleted = transactions.FirstOrDefault(t => t.Type == ArcTransactionType.QuestObjectiveCompleted);
    ...
    public async Task FullQuestFlow_AcceptToCompletion_Success()
    {
        ...
        Assert.Contains(transactions, t => t.Type == ArcTransactionType.QuestAccepted);
        Assert.Equal(3, transactions.Count(t => t.Type == ArcTransactionType.CharacterDefeated));
        Assert.Contains(transactions, t => t.Type == ArcTransactionType.QuestObjectiveCompleted);
        Assert.Contains(transactions, t => t.Type == ArcTransactionType.QuestStageAdvanced);
        Assert.Contains(transactions, t => t.Type == ArcTransactionType.QuestCompleted);
