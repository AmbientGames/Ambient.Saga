...
/// Verifies the end-to-end flow: commit a transaction in one arc,
...
public class CrossArcProgressTests : IDisposable
{
    ...
    private List<ArcTransaction> CreateTokenTransaction(string avatarId, string tokenRef) => new()
    {
        new ArcTransaction
        {
    ...
    private List<ArcTransaction> CreateQuestAcceptedTransaction(string avatarId, string questRef) => new()
    {
        new ArcTransaction
        {
            TransactionId = Guid.NewGuid(),
    ...
    private List<ArcTransaction> CreateReputationTransaction(string avatarId, string factionRef, int amount) => new()
    {
        new ArcTransaction
        {
            TransactionId = Guid.NewGuid(),
    ...
    private List<ArcTransaction> CreateCharacterDefeatedTransaction(string avatarId, string characterRef) => new()
    {
        new ArcTransaction
        {
            TransactionId = Guid.NewGuid(),
    ...
    private async Task<ArcInstance> CreateArcInstance(string arcRef)
    {
        return await _arcRepo.GetOrCreateInstanceAsync(_avatarId, arcRef);
    }
    ...
    {
        // Arrange: Create arc instance for ArcA
        ...
        await _arcRepo.AddAndCommitTransactionsAsync(arcAInstance.InstanceId, tokenTransactions);
    ...
    {
        // Arrange: Create arc instances for ArcA and ArcB
        var arcAInstance = await CreateArcInstance("ArcA");
        ...
        // Create a ArcTrigger with RequiresQuestTokenRef = ["KEY_TOKEN"]
    ...
    public async Task QuestAcceptedInArcA_DialogueChecksInArcB()
    {
        ...
        await _arcRepo.AddAndCommitTransactionsAsync(arcAInstance.InstanceId, questTransactions);
    ...
    public async Task ReputationEarnedAcrossMultipleArcs_SumsCorrectly()
    {
        ...
        await _arcRepo.AddAndCommitTransactionsAsync(arcAInstance.InstanceId, repTransactionsA);

        // Commit ReputationChanged +50 to ArcB
        var repTransactionsB = CreateReputationTransaction(_avatarId.ToString(), "GUILD_OF_MAGES", 50);
        await _arcRepo.AddAndCommitTransactionsAsync(arcBInstance.InstanceId, repTransactionsB);
    ...
    public async Task BossDefeatedInArcA_DialogueChecksCount()
    {
        ...
        await _arcRepo.AddAndCommitTransactionsAsync(arcAInstance.InstanceId, defeatTransactions);
