...
/// <summary>
/// End-to-End story tests for complete Arc flows.
///
...
public class SagaE2EStoryTests : IDisposable
{
    ...
    private World CreateTestWorld()
    {
        ...
        // Create Arc with two zones: merchant and boss
        var arc = new Arc
        {
        ...
        var bossTrigger = new ArcTrigger
        {
            RefName = "BossZone",
            EnterRadius = 30.0f,
            Spawn = new[]
        ...
        world.ArcTriggersLookup[arc.RefName] = new List<ArcTrigger> { merchantTrigger, bossTrigger };
    ...
    [Fact]
    public async Task CompleteArcStory_EnterZone_Dialogue_Trade_DefeatBoss_VerifyFinalState()
    {
        _logger.LogHeader("COMPLETE ARC STORY TEST");
        ...
        var arc = _world.ArcLookup["KagoshimaCastle"];
        var triggers = _world.ArcTriggersLookup["KagoshimaCastle"];
        ...
        _logger.LogEvent($"  - TriggerActivated: {allTransactions.Count(t => t.Type == ArcTransactionType.TriggerActivated)}");
        _logger.LogEvent($"  - CharacterSpawned: {allTransactions.Count(t => t.Type == ArcTransactionType.CharacterSpawned)}");
        _logger.LogEvent($"  - DialogueStarted: {allTransactions.Count(t => t.Type == ArcTransactionType.DialogueStarted)}");
        _logger.LogEvent($"  - ItemTraded: {allTransactions.Count(t => t.Type == ArcTransactionType.ItemTraded)}");
        _logger.LogEvent($"  - CharacterDefeated: {allTransactions.Count(t => t.Type == ArcTransactionType.CharacterDefeated)}");
    ...
    public async Task DeterminismTest_RunSameStoryTwice_IdenticalResults()
    {
        ...
        // Run the story twice with identical inputs (different avatars, different arc instances)
        var result1 = await RunCompleteStory("Run 1", Guid.NewGuid());

        // Clear database for second run to avoid conflicts
        _database.DropCollection("ArcInstances");
        _database.DropCollection("ArcTransactions");
    ...
    public async Task TransactionReplay_ExecuteStory_ThenReplayLog_ProducesSameState()
    {
        ...
        foreach (var tx in transactions)
        {
            ...
            if (tx.Type == ArcTransactionType.DialogueStarted)
            {
                _logger.LogEvent($"      DialogueTree: {tx.Data.GetValueOrDefault("DialogueTreeRef", "?")}");
            }
            else if (tx.Type == ArcTransactionType.ItemTraded)
            {
            ...
            else if (tx.Type == ArcTransactionType.CharacterDefeated)
            {
        ...
        _logger.LogEvent($"  Active triggers: {stateA.Triggers.Count(t => t.Value.Status == ArcTriggerStatus.Active)}");
        ...
        var stateMachine2 = new ArcStateMachine(arc, triggers, _world);
        ...
        _logger.LogEvent($"  Active triggers: {stateB.Triggers.Count(t => t.Value.Status == ArcTriggerStatus.Active)}");
    ...
    private class StoryResult
    {
        ...
        public Dictionary<ArcTransactionType, int> TransactionsByType { get; set; } = new();
