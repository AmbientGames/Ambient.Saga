...
public class QuestRewardsAndPrerequisitesTests : IDisposable
{
    ...
    private World CreateTestWorldWithQuests()
    {
        ...
        // transaction on the arc instance, like the dialogue ChangeReputation action)
    ...
    public async Task AdvanceQuestStage_WithStageRewards_AwardsCurrencyAndExperience()
    {
        ...
        var lootTransactions = new List<ArcTransaction>();
        for (var i = 0; i < 5; i++)
        {
            var lootTx = new ArcTransaction
            {
                TransactionId = Guid.NewGuid(),
                Type = ArcTransactionType.LootAwarded,
    ...
    public async Task AcceptQuest_WithQuestPrerequisite_SucceedsWhenPreviousQuestComplete()
    {
        ...
        var instance = await _repository.GetOrCreateInstanceAsync(avatar.Id, "TEST_ARC", CancellationToken.None);
        var questTransactions = new List<ArcTransaction>
        {
            new ArcTransaction
            {
                TransactionId = Guid.NewGuid(),
    ...
    public async Task ProgressQuestObjective_WithOnObjectiveReward_AwardsRewardForSpecificObjective()
    {
        ...
        await _repository.AddTransactionsAsync(instance.InstanceId, new List<ArcTransaction> { lootTx }, CancellationToken.None);
    ...
    public async Task ProgressQuestObjective_CompletingMultipleObjectives_AwardsSeparateRewards()
    {
        ...
        // Collect ITEM_A
        var lootTxA = new ArcTransaction
        {
            TransactionId = Guid.NewGuid(),
            Type = ArcTransactionType.LootAwarded,
            AvatarId = avatar.Id.ToString(),
        ...
        await _repository.AddTransactionsAsync(instance.InstanceId, new List<ArcTransaction> { lootTxA }, CancellationToken.None);
        ...
        // Collect ITEM_B
        var lootTxB = new ArcTransaction
        {
            TransactionId = Guid.NewGuid(),
            Type = ArcTransactionType.LootAwarded,
            AvatarId = avatar.Id.ToString(),
        ...
        await _repository.AddTransactionsAsync(instance.InstanceId, new List<ArcTransaction> { lootTxB }, CancellationToken.None);
    ...
    public async Task CompleteQuest_WithReputationReward_CommitsReputationChangedTransaction()
    {
        ...
        instance = await _repository.GetOrCreateInstanceAsync(avatar.Id, "TEST_ARC", CancellationToken.None);
        var reputationTx = instance.GetCommittedTransactions()
            .SingleOrDefault(t => t.Type == ArcTransactionType.ReputationChanged);
        ...
        // Replayed arc state sees the reputation
        var stateMachine = new ArcStateMachine(
            _world.ArcLookup["TEST_ARC"],
            _world.ArcTriggersLookup["TEST_ARC"],
            _world);
