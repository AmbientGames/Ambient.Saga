...
public class BattleCommandTests : IDisposable
{
    ...
    public async Task StartBattle_CreatesValidTransactions()
    {
        ...
        Assert.Contains(transactions, t => t.Type == ArcTransactionType.BattleStarted);

        // Verify enemy's opening turn was executed (enemy moves first)
        var turnTransactions = transactions.Where(t => t.Type == ArcTransactionType.BattleTurnExecuted).ToList();
        Assert.NotEmpty(turnTransactions);

        var battleStarted = transactions.First(t => t.Type == ArcTransactionType.BattleStarted);
    ...
    public async Task ExecuteBattleTurn_StateMatchesRecordedTurnResults_NoDoubleApplication()
    {
        ...
        var battleId = (await _repository.GetOrCreateInstanceAsync(avatarId, arcRef))
            ...
            .First(t => t.Type == ArcTransactionType.BattleStarted)
            .TransactionId;
    ...
    private World CreateBattleTestWorld()
    {
        ...
        var multiEnemyArc = new Arc
        {
            RefName = "MultiEnemyArc",
        ...
        var multiEnemyTrigger = new ArcTrigger
        {
        ...
        var world = new World
        {
            ...
            WorldTemplate = new WorldTemplate
            {
                ...
                {
                    Saga = new[] { weakBossArc, multiEnemyArc },
        ...
        world.ArcLookup[multiEnemyArc.RefName] = multiEnemyArc;
        world.ArcTriggersLookup[weakBossArc.RefName] = new List<ArcTrigger> { weakBossTrigger };
        world.ArcTriggersLookup[multiEnemyArc.RefName] = new List<ArcTrigger> { multiEnemyTrigger };
