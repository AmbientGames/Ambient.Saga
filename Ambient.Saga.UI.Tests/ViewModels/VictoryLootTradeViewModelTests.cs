...
/// - RefreshSagaStateAsync loads the saga-REPLAYED remaining inventory (the drop as
...
///   the victor exception in TradeItemHandler / SagaTransactionValidator accepts),
...
public class VictoryLootTradeViewModelTests
{
    ...
    /// <summary>
    /// Mediator stub that serves a replayed saga state containing the DEFEATED
    ...
    private sealed class VictoryLootStubMediator : IMediator
    {
        ...
        public Task<TResponse> Send<TResponse>(IRequest<TResponse> request, CancellationToken cancellationToken = default)
        {
            ...
            {
                var state = new SagaState { SagaRef = "TestSaga", Status = SagaStatus.Active };
    ...
    {
        var (vm, _, _) = CreateVictoryLootVm();
        var cacheVm = new MerchantTradeViewModel(new SagaInteractionContext
        {
    ...
    [Fact]
    public async Task RefreshSagaStateAsync_LoadsReplayedRemainingLoot_NotTemplateFallback()
    {
        var (vm, _, _) = CreateVictoryLootVm();
        ...
        // After the refresh the saga-REPLAYED remaining inventory wins: only the one
        // potion the victor hasn't taken yet — prior takes stay taken
        await vm.RefreshSagaStateAsync();
