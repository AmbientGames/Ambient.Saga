...
public class RealContentBattleE2ETests : IAsyncLifetime, IDisposable
{
    ...
    private const string SagaRef = "Aramatsurinomiya_to_ToyokeDaijinguOuterShrine_3";
    ...
    public async Task FullBattle_RealHostile_ToVictory_WithReactions_AndConsistentReconstructionAfterReload()
    {
        ...
        var arc = vm.CurrentWorld!.SagaArcLookup[SagaRef];
        ...
        var defeated = Assert.Single(committed, t => t.Type == SagaTransactionType.CharacterDefeated);
        ...
        var reloadedCommitted = (await _host.Repositories.SagaRepository.GetOrCreateInstanceAsync(avatar.AvatarId, SagaRef))
            .GetCommittedTransactions();
        Assert.Contains(reloadedCommitted, t => t.Type == SagaTransactionType.CharacterDefeated);
        Assert.True(_host.Repositories.AvatarProgressRepository.GetBossDefeatedCount(avatar.AvatarId, EnemyRef) >= 1);
    }
