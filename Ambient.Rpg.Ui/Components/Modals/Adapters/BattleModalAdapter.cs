...
public class BattleModalAdapter : IModal
{
    ...
    /// </summary>
    public event Action<RpgMainViewModel, CharacterViewModel>? VictoryLootAvailable;
    ...
    public void Render(object? context, ref bool isOpen)
    {
        ...
        RpgMainViewModel? viewModel = null;
    ...
    public void OnClosed()
    {
        ...
        // Closing the window SUSPENDS an un-ended battle; it deliberately sends no
        // flee/abandon command. A successful flee assigns the enemy the Disengaged
        // truce trait (it stops fighting) and can also fail its roll, consume a turn,
        // or eat the current telegraphed hit — none of which should happen just because
        // a player closed the window (often mid-telegraph, where the only exit was the
        // title-bar X). The server battle stays un-ended and is resumed on re-engage:
        // StartBattleHandler hands back the existing battle (re-presenting any pending
        // tell) and GetBattleStateHandler reconstructs the awaiting-reaction state, so
        // the reaction prompt is restored rather than the battle being left wedged.
        // Only the local modal state is reset here.
