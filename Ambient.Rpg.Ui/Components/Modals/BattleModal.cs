...
public class BattleModal
{
    ...
    /// </summary>
    private (RpgMainViewModel ViewModel, CharacterViewModel Character)? _victoryLootCandidate;

    /// <summary>
    /// Returns and clears the pending victory-loot candidate (null when the battle
    /// did not end in victory).
    /// </summary>
    public (RpgMainViewModel ViewModel, CharacterViewModel Character)? TakeVictoryLootCandidate()
    {
    ...
    private RpgMainViewModel? _cachedViewModel;
    ...
    public void Render(RpgMainViewModel viewModel, CharacterViewModel character, ModalManager modalManager, ref bool isOpen)
    {
        if (!isOpen)
        {
            Reset();
    ...
    private void RenderActiveBattle(RpgMainViewModel viewModel, CharacterViewModel character)
    {
    ...
    private void RenderActionButtons(RpgMainViewModel viewModel, CharacterViewModel character)
    {
        if (_currentState == null) return;
    ...
    /// </summary>
    private void RenderReactionPanel(RpgMainViewModel viewModel, CharacterViewModel character)
    {
    ...
    private void SelectReaction(AvatarDefenseType reaction, RpgMainViewModel viewModel, CharacterViewModel character)
    {
    ...
    /// </summary>
    private bool TryStartReactionFromResult(RpgMainViewModel viewModel, ArcCommandResult result)
    {
        if (result.Data == null ||
    ...
    private BattleEngine? CreateReactionEngine(RpgMainViewModel viewModel, string tellRefName, float baseDamage)
    {
    ...
    private async Task ResolveReactionAsync(RpgMainViewModel viewModel, CharacterViewModel character, AvatarDefenseType reaction)
    {
    ...
    private void RenderBattleEnded(RpgMainViewModel viewModel, CharacterViewModel character, ModalManager modalManager)
    {
        if (_currentState == null) return;
    ...
    private async Task InitializeBattleAsync(RpgMainViewModel viewModel, CharacterViewModel character)
    {
        ...
        try
        {
            ...
            // Recruited party members fight alongside the avatar. Each party member
            // references a character template; resolve it (same lookup used for the
            // enemy above) and hand the Character to BattleSetup, which builds the
            // companion Combatant — stats, equipment, stance — exactly like the enemy.
            // The engine's (tested) companion turn-loop then runs them. A member whose
            // character ref no longer resolves (character removed from the world) is
            // skipped rather than crashing the battle.
            var partyMembers = viewModel.Avatar.Party?.Member;
            if (partyMembers != null)
            {
                foreach (var member in partyMembers)
                {
                    if (string.IsNullOrEmpty(member.CharacterRef))
                        continue;

                    var companionTemplate = viewModel.CurrentWorld.Gameplay?.Characters?
                        .FirstOrDefault(c => c.RefName == member.CharacterRef);
                    if (companionTemplate == null)
                    {
                        System.Diagnostics.Debug.WriteLine($"[BattleModal] Skipping party member with unresolved character ref: {member.CharacterRef}");
                        continue;
                    }

                    battleSetup.CompanionCharacters.Add(companionTemplate);
                }
            }
            ...
            // Companion combatants BattleSetup built from the party (empty when solo);
            // passed to the command so the server-side engine runs their turns too.
    ...
    private async Task ExecuteTurnAsync(RpgMainViewModel viewModel, CharacterViewModel character, CombatAction action)
    {
        if (viewModel.Avatar == null || _battleInstanceId == Guid.Empty)
            return;
    ...
    private async Task RefreshBattleStateAsync(RpgMainViewModel viewModel, CharacterViewModel character)
    {
        if (viewModel.Avatar == null || _battleInstanceId == Guid.Empty)
            return;
    ...
    /// (OnVictory/OnDefeat).
    /// </summary>
    private void CheckBattleDialogueTriggers(RpgMainViewModel viewModel, CharacterViewModel character)
    {
    ...
    private void OpenSpellSelectionModal(RpgMainViewModel viewModel)
    {
    ...
    private void OpenItemSelectionModal(RpgMainViewModel viewModel)
    {
        if (_currentState?.AvatarCombatant == null || viewModel.CurrentWorld == null) return;
    ...
    private void OpenEquipmentChangeModal(RpgMainViewModel viewModel)
    {
        if (_currentState?.AvatarCombatant == null || viewModel.CurrentWorld == null) return;
    ...
    private void OpenAffinityChangeModal(RpgMainViewModel viewModel)
    {
        if (_currentState?.AvatarCombatant == null || viewModel.CurrentWorld == null) return;
    ...
    private void OpenStanceChangeModal(RpgMainViewModel viewModel)
    {
        if (_currentState?.AvatarCombatant == null || viewModel.CurrentWorld == null) return;
    ...
    /// </summary>
    private async Task ApplyDialogueEffectsAsync(RpgMainViewModel viewModel, CharacterViewModel character,
        List<Ambient.Rpg.Engine.Domain.Dialogue.Events.DialogueSystemEvent> effects)
    {
        if (viewModel.Avatar == null || _battleInstanceId == Guid.Empty)
            return;
        ...
        foreach (var effect in effects)
        {
            switch (effect)
            {
                case Ambient.Rpg.Engine.Domain.Dialogue.Events.HealSelfEvent heal:
                    // Content authors percent (e.g. Amount="50"); battle stats are 0-1
                    healAmount += heal.Amount / 100f;
                    break;
                case Ambient.Rpg.Engine.Domain.Dialogue.Events.ChangeStanceEvent stance:
                    stanceRef = stance.StanceRef;
                    break;
                case Ambient.Rpg.Engine.Domain.Dialogue.Events.ChangeAffinityEvent affinity:
                    affinityRef = affinity.AffinityRef;
                    break;
                case Ambient.Rpg.Engine.Domain.Dialogue.Events.EndBattleEvent end:
    ...
    private void OpenMidBattleDialogue(RpgMainViewModel viewModel, CharacterViewModel character)
    {
    ...
    private void RenderSelectionModals(RpgMainViewModel viewModel)
    {
