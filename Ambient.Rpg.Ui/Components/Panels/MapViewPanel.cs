...
/// <summary>
/// Center panel showing the map view with arcs and characters
...
public class MapViewPanel
{
    ...
    public void Render(RpgMainViewModel viewModel, nint heightMapTexturePtr, int heightMapWidth, int heightMapHeight, ModalManager modalManager)
    {
        ...
        if (heightMapTexturePtr != nint.Zero)
        {
            ...
            // Overlay arcs, characters, and avatar on the heightmap
            ...
            // Draw Arc zones (proximity trigger rings)
            foreach (var arc in viewModel.Arcs)
            {
                if (!arc.IsVisibleOnMap)
                    continue;
            ...
            // Draw Arc feature center dots (always visible at Arc centers)
            foreach (var arc in viewModel.Arcs)
            {
                if (!arc.IsVisibleOnMap)
                    continue;

                var center = PixelToScreen(arc.PixelX, arc.PixelY);
                ...
                var color = arc.FeatureDotColor;
                var opacity = (float)arc.FeatureDotOpacity;
                ...
                if (distance <= dotRadius + 3) // 3px tolerance for easier hovering
                {
                    ...
                    {
                        var tooltip = $"{arc.DisplayName}\n" +
                                      $"Status: {arc.InteractionStatus}\n" +
                                      $"Category: {arc.Category}\n" +
                                      $"Ref: {arc.RefName}";

                        if (arc.RequiresQuestTokens?.Length > 0)
                            tooltip += $"\nRequires: {string.Join(", ", arc.RequiresQuestTokens)}";

                        if (arc.GivesQuestTokens?.Length > 0)
                            tooltip += $"\nGives: {string.Join(", ", arc.GivesQuestTokens)}";

                        ImGui.SetTooltip(tooltip);
                    }
                    else
                    {
                        ImGui.SetTooltip(arc.DisplayName);
                    }

                    // Handle click - quests are now handled via character dialogue, not direct signpost interaction
                    // Category is just a visual hint; actual interaction happens through ArcTrigger spawned characters
                }
            }
            ...
            // Characters is only ever replaced wholesale (atomic publish in RpgMainViewModel;
        ...
        ImGui.Spacing();
        ImGui.Separator();
        ImGui.TextColored(new Vector4(0.7f, 0.7f, 0.7f, 1), $"Arcs: {viewModel.Arcs.Count}");
        ...
        if (viewModel.Characters.Count == 0)
        {
            ...
            ImGui.TextWrapped("Move into Arc zones to spawn characters");
            ImGui.PopStyleColor();
        }
    ...
    /// </summary>
    private void RenderTeleportConfirmPopup(RpgMainViewModel viewModel)
    {
    ...
    private async Task TeleportAvatarAsync(RpgMainViewModel viewModel, double lat, double lon, double pixelX, double pixelY, int cost)
    {
        try
        {
