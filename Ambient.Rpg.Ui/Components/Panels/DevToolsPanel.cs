    ...
    public void Render(RpgMainViewModel viewModel, ModalManager modalManager)
    {
        ...
        if (ImGui.CollapsingHeader("Debug Info"))
        {
            ...
            if (viewModel.CurrentWorld != null)
            {
                ...
                ImGui.Text($"Arcs: {viewModel.CurrentWorld.Gameplay?.Saga?.Length ?? 0}");
        ...
        // Transaction Log Section (for Arc debugging)
        ImGui.Spacing();
        if (ImGui.CollapsingHeader("Arc Debug"))
        {
            ImGui.Indent(10 * UIConstants.DpiScale);

            if (viewModel.Arcs?.Count > 0)
            {
                foreach (var arc in viewModel.Arcs.Take(5))
                {
                    ImGui.BulletText($"{arc.DisplayName}");
                }

                if (viewModel.Arcs.Count > 5)
                {
                    ImGui.TextColored(UIColors.TextDisabled, $"... and {viewModel.Arcs.Count - 5} more");
                }
            }
            else
            {
                ImGui.TextColored(UIColors.TextDisabled, "No active arcs");
            }
