...
public class InventoryPanel
{
    ...
    /// </summary>
    private void RenderConsumables(RpgMainViewModel viewModel, ItemCollection caps)
    {
    ...
    /// </summary>
    private void RenderSpells(RpgMainViewModel viewModel, ItemCollection caps)
    {
    ...
    // Quest tokens now live in arc state (AwardedQuestTokens), not avatar inventory.
    // TODO: Render from arc state read model when UI plumbing is available.
    private void RenderQuestTokens(RpgMainViewModel viewModel, ItemCollection caps)
    {
    }
    ...
    /// </summary>
    private void RenderAffinity(RpgMainViewModel viewModel, ItemCollection caps)
    {
    ...
    /// </summary>
    private void RenderStance(RpgMainViewModel viewModel, ItemCollection caps)
    {
        var world = viewModel.CurrentWorld;
    ...
    // Server-authoritative price — SharpenToolHandler rejects any other value
    private const int SharpenCost = Ambient.Rpg.Engine.Domain.ToolSharpening.CostCredits;

    /// <summary>
    /// Renders the Tools section with expandable details.
    /// </summary>
    private void RenderTools(RpgMainViewModel viewModel, ItemCollection caps)
    {
                ...
                foreach (var tool in caps.Tools)
                {
                    ...
                    if (treeNodeOpen)
                    {
                        ...
                        if (toolDef != null)
                        {
                            ...
                            if (toolDef.EffectiveSubstances != null && toolDef.EffectiveSubstances.Length > 0)
                            {
                                ...
                                foreach (var substance in toolDef.EffectiveSubstances)
                                {
                                    var multiplierText = substance.EffectivenessMultiplier != 1f
                                        ...
                                        : "";
                                    ImGui.BulletText($"{substance.Substance}{multiplierText}");
                                }
                                ImGui.Spacing();
                            }
                            ...
                            ImGui.TextColored(new Vector4(0.5f, 1, 0.5f, 1), $"Value: {toolDef.BaseValue}");
                        }
    ...
    /// </summary>
    private void RenderBlocks(RpgMainViewModel viewModel, ItemCollection caps)
    {
                ...
                foreach (var stack in stacks)
                {
                    ...
                    if (treeNodeOpen)
                    {
                        ...
                        if (blockDef != null)
                        {
                            ...
                            // Substance
                            ImGui.TextColored(new Vector4(0.5f, 0.8f, 1f, 1), $"Substance: {blockDef.Substance}");

                            // Value
                            ImGui.TextColored(new Vector4(0.5f, 1, 0.5f, 1), $"Value: {blockDef.BaseValue}");
    ...
    /// </summary>
    private void EnsureBlockCache(RpgMainViewModel viewModel)
    {
    ...
    /// </summary>
    private void RenderMaterials(RpgMainViewModel viewModel, ItemCollection caps)
    {
                    ...
                    if (treeNodeOpen)
                    {
                        if (materialItem != null)
                        {
                            ...
                            if (materialItem.CompatibleSubstances != null && materialItem.CompatibleSubstances.Length > 0)
                            {
                                ...
                                {
                                    ImGui.BulletText(substance.Substance.ToString());
                                }
                                ImGui.Spacing();
                            }

                            ImGui.TextColored(new Vector4(0.5f, 1, 0.5f, 1), $"Value: {materialItem.BaseValue}");
    ...
    private async Task EquipItemAsync(RpgMainViewModel viewModel, string equipmentRef, string slotRef)
    {
        try
        {
    ...
    private async Task UnequipItemAsync(RpgMainViewModel viewModel, string equipmentRef, string slotRef)
    {
        try
        {
    ...
    private async Task UseConsumableAsync(RpgMainViewModel viewModel, string consumableRef)
    {
        try
        {
    ...
    private async Task SharpenToolAsync(RpgMainViewModel viewModel, string toolRef)
    {
        try
        {
    ...
    /// </summary>
    private void RenderEquipmentBySlot(RpgMainViewModel viewModel, ItemCollection caps)
    {
        var world = viewModel.CurrentWorld;
    ...
    private static (EquipmentEntry entry, Equipment? def) FindEquippedItem(
        RpgMainViewModel viewModel, List<(EquipmentEntry entry, Equipment? def)> slotEquipment)
    {
    ...
    /// </summary>
    private void RenderEquipmentItem(RpgMainViewModel viewModel, EquipmentEntry equip, Equipment? equipItem, string slotRef)
    {
    ...
    /// </summary>
    private void RenderHotbarAssignPopup(RpgMainViewModel viewModel)
    {
    ...
    private string GetSlotItemName(RpgMainViewModel viewModel, HotbarSlot slot)
    {
    ...
    private void DiscardEquipment(RpgMainViewModel viewModel, string equipmentRef, string displayName)
    {
        var caps = viewModel.Avatar?.Capabilities;
    ...
    private void DiscardConsumable(RpgMainViewModel viewModel, string consumableRef, string displayName)
    {
        var caps = viewModel.Avatar?.Capabilities;
    ...
    private void DiscardSpell(RpgMainViewModel viewModel, string spellRef, string displayName)
    {
        var caps = viewModel.Avatar?.Capabilities;
    ...
    private void DiscardTool(RpgMainViewModel viewModel, string toolRef, string displayName)
    {
        var caps = viewModel.Avatar?.Capabilities;
    ...
    private void DiscardBlock(RpgMainViewModel viewModel, string blockRef, string displayName)
    {
    ...
    private void DiscardMaterial(RpgMainViewModel viewModel, string materialRef, string displayName)
    {
        var caps = viewModel.Avatar?.Capabilities;
