...
/// Event-sourced: Reads from ArcState (replayed from transaction log).
/// </summary>
public partial class QuestLogViewModel : ObservableObject
{
    private ArcInteractionContext _context;
    ...
    public QuestLogViewModel(ArcInteractionContext context, IMediator mediator)
    {
        _context = context;
    ...
    /// <summary>
    /// Refreshes the quest lists from ArcState (event-sourced).
    ...
    public async Task RefreshQuestsAsync()
    {
        ...
        if (_context.AvatarEntity == null || _context.World?.Gameplay?.Saga == null)
        {
        ...
        // Query all Arcs for quest state
        foreach (var arc in _context.World.Gameplay.Saga)
        {
            try
            {
                // Get Arc state from transaction log
                ...
                if (arcState == null) continue;

                // Extract active quests
                foreach (var (questRef, questState) in arcState.ActiveQuests)
                {
                ...
                foreach (var questRef in arcState.CompletedQuests)
                {
                    var quest = _context.World.TryGetQuestByRefName(questRef);
                    if (quest == null) continue;
            ...
            {
                // Skip this arc if state query fails
                continue;
            }
        }
