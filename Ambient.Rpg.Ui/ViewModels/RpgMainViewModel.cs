...
using Ambient.Rpg.Engine.Application.Queries.Loading;
...
using static Ambient.Rpg.Ui.Services.HeightMapProcessor;
using SpawnDevCharacterCommand = Ambient.Rpg.Engine.Application.Commands.Arcs.SpawnDevCharacterCommand;
using System.Runtime.InteropServices;
using Ambient.Rpg.Rendering.DirectX;

namespace Ambient.Rpg.Presentation.UI.ViewModels;

public partial class RpgMainViewModel : ObservableObject
{
    ...
    [ObservableProperty]
    private ObservableCollection<ArcViewModel> _arcs = new();
    ...
    /// <summary>
    /// Recent transactions from all active arc instances.
    /// Populated during background processing for Journal History tab.
    /// </summary>
    [ObservableProperty]
    private ObservableCollection<Ambient.Rpg.Engine.Domain.Arcs.TransactionLog.ArcTransaction> _recentTransactions = new();
    ...
    [ObservableProperty]
    private bool _isReadyForArcProcessing;
    ...
    [ObservableProperty]
    private bool _isArcInteractionActive = false;
    ...
    // Context that ArcInteractionViewModel references.
    ...
    private ArcInteractionContext _arcInteractionContext = new()
    {
        World = null!,
        AvatarEntity = null!,
    ...
    {
        _arcInteractionContext.World = value;
    }

    partial void OnAvatarChanged(AvatarEntity? value)
    {
        _arcInteractionContext.AvatarEntity = value;
        _arcInteractionContext.AvatarId = value?.AvatarId ?? Guid.Empty;
    }
    ...
    /// <summary>
    /// Invalidates the cached character states for an arc, forcing a re-query on the next check.
    /// Call after transactions (trade, combat, dialogue) or server sync.
    /// </summary>
    public void InvalidateCharacterCache(string? arcRef = null)
    {
        if (arcRef != null)
            _cachedCharacterStates.Remove(arcRef);
        else
    ...
    private ArcCategory? _currentEntityCategory;

    // Track current arc/character context for CQRS commands
    private string? _currentArcRef;

    // Track Arcs where Arc-level spawns have already occurred (to avoid duplicates)
    private readonly HashSet<string> _spawnedArcs = new();
    ...
    private readonly ArcInstanceRepositoryProvider _repositoryProvider;
    ...
    public RpgMainViewModel(
        WorldProvider worldProvider,
        ArcInstanceRepositoryProvider repositoryProvider,
        ...
        [Microsoft.Extensions.DependencyInjection.FromKeyedServicesAttribute("imgui")] IArchetypeSelector imguiArchetypeSelector)
    {
        ...
        MerchantTrade = new MerchantTradeViewModel(_arcInteractionContext, _mediator);
        ...
        QuestLog = new QuestLogViewModel(_arcInteractionContext, _mediator);
        Achievements = new AchievementViewModel(_arcInteractionContext);
    ...
    private void CheckEntityRespawns()
    {
        ...
        // Character availability is computed from ArcState:
        //   IsAvailable = (!IsAlive && DefeatedAt + RespawnDurationSeconds <= Now)
        // ArcStateMachine handles this automatically during replay.
    ...
    private async Task CheckAvailableInteractionsAsync()
    {
        ...
        try
        {
            ...
            // Get nearby arcs RefNames for efficient filtering
            var nearbyArcRefs = ArcProximityService.FilterArcsByProximity(
                avatarModelX, avatarModelZ, CurrentWorld)
                .Select(s => s.Arc.RefName)
                .ToHashSet();

            //System.Diagnostics.Debug.WriteLine($"[CheckInteractions] Found {nearbyArcRefs.Count} nearby Arcs (of {_arcs.Count} total)");

            // Query spawned characters for ALL nearby Arcs (not just visible ones)
            // Characters should always be visible regardless of arc visibility
            // Remove cached entries for arcs no longer nearby
            var staleRefs = _cachedCharacterStates.Keys.Where(k => !nearbyArcRefs.Contains(k)).ToList();
            foreach (var staleRef in staleRefs)
                _cachedCharacterStates.Remove(staleRef);

            foreach (var arcRef in nearbyArcRefs)
            {
                // Get Arc template for center coordinates
                if (!CurrentWorld.ArcLookup.TryGetValue(arcRef, out var arcTemplate))
                    continue;

                List<CharacterState> characterStates;

                // Use cached character states if available (skip expensive replay)
                //if (_cachedCharacterStates.TryGetValue(arcRef, out var cached))
                //{
                ...
                //{
                    ...
                    _cachedCharacterStates[arcRef] = characterStates;
                //}
                ...
                foreach (var characterState in characterStates)
                {
                    ...
                    var worldLon = CoordinateConverter.ArcRelativeXToLongitude(
                        characterState.CurrentLongitudeX, arcTemplate.Longitude, CurrentWorld);
                    var worldLat = CoordinateConverter.ArcRelativeZToLatitude(
                        characterState.CurrentLatitudeZ, arcTemplate.Latitude, CurrentWorld);
    ...
    /// Uses the arbiter query to select ONE interaction across all Arcs.
    /// Internal for tests (see RpgMainViewModelInitiatedInteractionTests).
    /// </summary>
    internal async Task CheckForInitiatedInteractionsAsync()
    {
        ...
        //   ApproachRadius IS the interaction). The 3D host games do NOT subscribe:
        ...
        try
        {
            ...
            if (result.HasInteraction && result.Character != null)
            {
                ...
                if (result.IsAssault && AssaultRequested != null)
                {
                    if (!_isInDialogue)
                    {
                        RaiseAssaultRequested(result.ArcRef, result.Character.CharacterInstanceId);
                    }
                }
                ...
                {
                    await StartDialogueWithCharacterAsync(result.ArcRef, result.Character.CharacterInstanceId);
                }
            }
        }
        catch (Exception ex)
    ...
    /// </summary>
    private void RaiseAssaultRequested(string arcRef, Guid characterInstanceId)
    {
        var characterVM = Characters.FirstOrDefault(c =>
            c.CharacterInstanceId == characterInstanceId && c.ArcRef == arcRef);

        if (characterVM == null)
        {
            System.Diagnostics.Debug.WriteLine($"*** Assault deferred: CharacterViewModel not published yet for {characterInstanceId} in Arc {arcRef}");
            return;
        }
    ...
    private string? _currentDialogueArcRef;
    ...
    /// </summary>
    private async Task StartDialogueWithCharacterAsync(string arcRef, Guid characterInstanceId)
    {
        if (Avatar == null)
            return;

        // Don't re-open dialogue if already in dialogue with this character
        if (_isInDialogue && _currentDialogueArcRef == arcRef && _currentDialogueCharacterInstanceId == characterInstanceId)
        {
            return;
        }
        ...
        try
        {
            System.Diagnostics.Debug.WriteLine($"*** Starting dialogue with character {characterInstanceId} in Arc {arcRef}");

            // Store for later use and mark as in dialogue
            _currentDialogueArcRef = arcRef;
            ...
            var state = await GetDialogueStateAsync(arcRef, characterInstanceId);
            ...
            {
                System.Diagnostics.Debug.WriteLine($"*** WARNING: CharacterViewModel not found for {characterInstanceId} in Arc {arcRef}");
            }
        }
        catch (Exception ex)
        {
    ...
    private async Task<DialogueStateResult?> GetDialogueStateAsync(string arcRef, Guid characterInstanceId)
    {
        if (Avatar == null)
            return null;
    ...
    {
        if (!_isInDialogue || Avatar == null || _currentDialogueArcRef == null)
        {
            _isInDialogue = false;
            _currentDialogueArcRef = null;
            _currentDialogueCharacterInstanceId = Guid.Empty;
            return;
        }

        var arcRef = _currentDialogueArcRef;
    ...
    /// Sets up world, database, avatar, height map, Arcs, and triggers.
    /// </summary>
    private async Task InitializeWorldCoreAsync(IWorld world, string dataDirectory)
    {
        ...
        // Subscribers must set IsReadyForArcProcessing = true when ready
        ...
        // Load Arcs and triggers from world with feature status (progress repo
        // set by InitializeWorldDatabase above; needed for cross-arc token gates)
        var (arcs, triggers) = await ArcViewModel.LoadFromWorldAsync(world, Avatar, _worldRepository, _avatarProgressRepositoryProvider.Repository);
        Arcs.Clear();
        AllTriggers.Clear();
    ...
    /// <summary>
    /// Loads recent transactions from all arc instances for the current avatar.
    ...
    private async Task LoadRecentTransactionsAsync()
    {
        if (Avatar == null)
        {
            RecentTransactions = new ObservableCollection<Ambient.Rpg.Engine.Domain.Arcs.TransactionLog.ArcTransaction>();
            return;
        }
        ...
        try
        {
            ...
            RecentTransactions = new ObservableCollection<Ambient.Rpg.Engine.Domain.Arcs.TransactionLog.ArcTransaction>(allTransactions);
        }
        catch (InvalidOperationException)
        {
    ...
    private async Task LoadHeightMapImageInternalAsync(IWorld world, string dataDirectory)
    {
        ...
        {
            // Generate synthetic map from arc locations
    ...
    /// <summary>
    /// Generates a synthetic map for procedural worlds based on arc locations.
    /// </summary>
    private async Task LoadProceduralMapAsync(IWorld world)
    {
        try
        {
            ...
            // Calculate arc count for info display
            var arcCount = world.ArcLookup.Count;
            ...
            AddToastMessage($"Procedural map generated ({arcCount} arc arcs)", MessageType.Quest);
        }
        catch (Exception ex)
        {
    ...
    private void InitializeWorldDatabase(IWorld world)
    {
        try
        {
            ...
            _repositoryProvider.SetRepository(repositories.ArcRepository);
    ...
    {
        if (CurrentWorld == null || Avatar == null || !IsReadyForArcProcessing)
            return false;
        ...
        try
        {
            ...
            if (result.Successful)
            {
                ...
                InvalidateCharacterCache(trigger.ArcRefName);

                // Refresh trigger status so completed triggers disappear
                await UpdateArcFeatureStatus(trigger.ArcRefName);
            }
            else
            {
    ...
    /// <summary>
    /// Updates a ArcViewModel's trigger status after recording a transaction.
    /// </summary>
    private async Task UpdateArcFeatureStatus(string arcRef)
    {
        var arcVM = Arcs.FirstOrDefault(s => s.RefName == arcRef);
        if (arcVM != null && CurrentWorld != null)
        {
            var arc = CurrentWorld.Gameplay?.Saga?.FirstOrDefault(s => s.RefName == arcRef);
            if (arc != null)
            {
                // Convert Arc GPS to model coordinates for query
                var arcModelX = CoordinateConverter.LongitudeToModelX(arc.Longitude, CurrentWorld);
                var arcModelZ = CoordinateConverter.LatitudeToModelZ(arc.Latitude, CurrentWorld);
    ...
    /// Characters should only spawn via Arc triggers (which create CharacterSpawned transactions).
    /// To enable manual spawning, would need to:
    /// 1. Find nearest Arc to avatar
    /// 2. Create CharacterSpawned transaction in that Arc's transaction log
    /// 3. Query ArcState to get the spawned character
    /// </summary>
    [RelayCommand]
    private void SpawnCharacterAtAvatar(string characterRef)
    {
        AddToastMessage("Manual character spawning disabled - characters spawn via Arc triggers only", MessageType.Warning);
    ...
    private async Task<List<ProximityTriggerViewModel>> FindTriggersAtPoint(double modelX, double modelZ)
    {
        ...
        var triggerInteractions = interactions
            .Where(i => i.Type == ArcInteractionType.ArcTrigger)
            .ToList();

        // Step 1: Reset ALL triggers on ALL arcs
        foreach (var arc in Arcs)
        {
            foreach (var trigger in arc.Triggers)
            {
        ...
        foreach (var ti in triggerInteractions)
        {
            var trigger = Arcs
                ...
                .FirstOrDefault(t => t.RefName == ti.ArcTriggerRef && t.ArcRefName == ti.ArcRef);

            // If trigger not found, the arc may be hidden - discover it now
            if (trigger == null && CurrentWorld != null)
            {
                // Try to load the hidden arc and add it to ViewModels (discovery!)
                if (CurrentWorld.ArcLookup.TryGetValue(ti.ArcRef, out var arc) &&
                    CurrentWorld.ArcTriggersLookup.TryGetValue(ti.ArcRef, out var arcTriggers) &&
                    CurrentWorld.HeightMapMetadata != null)
                {
                    var newArcVM = ArcViewModel.FromDomain(
                        arc,
                        arcTriggers.OrderByDescending(t => t.EnterRadius).ToList(),
                        CurrentWorld.HeightMapMetadata,
                        CurrentWorld);

                    // Set feature dot color based on category (newly discovered = available)
                    newArcVM.InteractionStatus = InteractionStatus.Available;
                    newArcVM.FeatureDotColor = ArcColors.GetColor(newArcVM.Category, InteractionStatus.Available);
                    newArcVM.FeatureDotOpacity = 1.0;

                    Arcs.Add(newArcVM);

                    // Add triggers to AllTriggers for rendering
                    foreach (var triggerVM in newArcVM.Triggers)
                    {
                        triggerVM.ArcRefName = newArcVM.RefName;
                        AllTriggers.Add(triggerVM);
                    }

                    // Now find the trigger again
                    trigger = newArcVM.Triggers.FirstOrDefault(t => t.RefName == ti.ArcTriggerRef);

                    System.Diagnostics.Debug.WriteLine($"[Discovery] Arc '{ti.ArcRef}' discovered and added to ViewModels");
                }
            }
            ...
            {
                var arc = CurrentWorld.TryGetArcByRefName(ti.ArcRef);
                var actualTrigger = arc?.ArcTrigger?.FirstOrDefault(t => t.RefName == ti.ArcTriggerRef);
    ...
    private async Task LoadOrCreateAvatarAsync(IWorld world)
    {
        ...
        // Generate the avatar GUID here so it's consistent across local DB and any consumer of arc
    ...
    public async Task<bool> EquipItemAsync(string? equipmentRef, string? slotRef = null)
    {
        ...
        try
        {
            ...
            // Use first arc in the world for transaction logging, or a default
            var arcRef = CurrentWorld.Gameplay?.Saga?.FirstOrDefault()?.RefName ?? "AvatarLife";
    ...
    /// Creates proper arc transactions so dialogue/trade/combat work correctly.
    /// </summary>
    public async Task<CharacterViewModel?> SpawnDevCharacterAsync(DevCharacterType characterType)
    {
        ...
        try
        {
            ...
            // Find a real arc to use
            var arcRef = FindArcWithCharacter(testCharacter.RefName)
                ?? CurrentWorld.Gameplay?.Saga?.FirstOrDefault()?.RefName
                ?? "";
    ...
    /// <summary>
    /// Finds an arc that spawns the given character (for dev testing with real arc refs).
    /// </summary>
    private string? FindArcWithCharacter(string characterRef)
    {
        var arcs = CurrentWorld?.Gameplay?.Saga;
        if (arcs == null)
            return null;

        foreach (var arc in arcs)
        {
            // Check if this arc has triggers that spawn the character
            if (CurrentWorld!.ArcTriggersLookup.TryGetValue(arc.RefName, out var triggers))
            {
        ...
        // Fallback: just return first arc (dialogue needs an arc context)
        return arcs.FirstOrDefault()?.RefName;
    }
